...
- `code_mode=True` tags every tool with `code_mode=True` metadata for [`CodeMode`](code-mode.md)`(tools={'code_mode': True})`.
- Wrap write actions (`*_create_*`, `*_update_*`, `*_delete_*`) with [tool approval](/ai/tools-toolsets/deferred-tools/#human-in-the-loop-tool-approval).
- All configuration is [agent-spec](/ai/core-concepts/agent-spec/) expressible; keep the API key in the environment, not the spec file:
